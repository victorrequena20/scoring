...
export function BookingAvailable({ title, subheader, chart, ...other }: Props) {
  ...
  const chartSeries = (chart.series.filter((i) => i.label === 'Pagado')[0].value / total) * 100;
    ...
    plotOptions: {
      radialBar: {
        ...
        dataLabels: {
          name: { offsetY: -12 },
          value: { offsetY: 6 },
          total: { label: 'Puntaje', formatter: () => fNumber(total) },
        },
      },
    },
    ...chart.options,
          ...
          >
            <Box
              sx={{
                ...
                ...(item.label === 'Pagado' && { bgcolor: chartColors[1] }),
              }}
            />
